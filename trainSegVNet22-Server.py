# 保存下dice最小的model_stat，以便生成新的mask
import os
...
## 130个数据中用5折交叉验证进行训练后的结果验证，这里idxs应该是提前生成的随机序列
...
#                loss += AtriaSeg2018.dice_loss(output, target).item()
            ...
            if is_output_test_images and epoch % 5 == 0:
                ...
                o = data.view(-1, fixed_size[0], fixed_size[1], fixed_size[2], 1)
                imageio.imsave('Images/Test/o-{}.jpg'.format(i),
                                  o[0, :, :, fixed_size[2] // 2, 0].cpu().numpy().T) # .astype(uint8)

                t = target.view(-1, fixed_size[0], fixed_size[1], fixed_size[2], 1).float() * o.max() / 3 + o
                imageio.imsave('Images/Test/t-{}.jpg'.format(i),
                                  t[0, :, :, fixed_size[2] // 2, 0].cpu().numpy().T)

                pred = output.data.max(1)[1]  # get the index of the max log-probability
                # print(pred.shape)
                x = pred.view(-1, fixed_size[0], fixed_size[1], fixed_size[2], 1).float() * o.max() / 3 + o
                imageio.imsave('Images/Test/p-{}-{}.jpg'.format(i, epoch),
                ...
                imageio.imsave('Images/Test/p-{}-{}-1.jpg'.format(i, epoch),
                                  x[0, :, :, fixed_size[2] // 2, 0].cpu().numpy().T)

                pred = out2.data.max(1)[1]  # get the index of the max log-probability
                x = pred.view(-1, fixed_size[0], fixed_size[1], fixed_size[2], 1).float()
                imageio.imsave('Images/Test/p-{}-{}-2.jpg'.format(i, epoch),
                                  x[0, :, :, fixed_size[2] // 2, 0].cpu().numpy().T)

                pred = out3.data.max(1)[1]  # get the index of the max log-probability
                x = pred.view(-1, fixed_size[0], fixed_size[1], fixed_size[2], 1).float()
                imageio.imsave('Images/Test/p-{}-{}-3.jpg'.format(i, epoch),
                                  x[0, :, :, fixed_size[2] // 2, 0].cpu().numpy().T)
                """

            output = output.view(-1, fixed_size[0], fixed_size[1], fixed_size[2], 2) # 1,144,144,48,2
            target = target.view(-1, fixed_size[0], fixed_size[1], fixed_size[2]) # 1,144,144,48
